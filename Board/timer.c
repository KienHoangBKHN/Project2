 ...
 *
 * \11/04/2018 Kien Hoang (chinh sua ham void TimerStart( TimerEvent_t *obj );)
 *  	TimerStop();
 * chua dinh nghia ham void TimerLowPowerHandler( void );
 */
#include "board.h"
#include "rtc-board.h"
#include "timer.h"
...
void TimerLowPowerHandler( void )
{
    if( ( TimerListHead != NULL ) && ( TimerListHead->IsRunning == true ) )
    {
        ...
        {
            HasLoopedThroughMain = 0;
            if( GetBoardPowerSource( ) == BATTERY_POWER )  // problem
            {
